...
`OssPlay/docs` (reserved in the original repo-strategy decision below, not created until now) was scaffolded during a large `ossplay` auth-expansion pass, once the user redirected away from a plan that deferred all documentation to a single trailing step — the new standing rule is that docs land alongside the feature that motivates them, not in a batch afterward (recorded in this session's assistant memory too, so it applies to `ossplay` work generally, not just this repo).

- **Fumadocs** (Next.js + `fumadocs-mdx` + `fumadocs-ui`), scaffolded by hand rather than via `create-fumadocs-app` — the CLI's interactive TUI prompts don't work over a non-TTY automation shell, so the standard file layout (`source.config.ts`, `lib/source.ts`, `app/docs/[[...slug]]/page.tsx`, orama search route) was written directly, cross-checked against the installed packages' own `.d.ts` files rather than assumed from memory.
- **Fixed three-category taxonomy, decided upfront to avoid reorganizing later**: `getting-started/` (install/deploy/first-run, ordered as a path), `guides/` (task-oriented how-tos, one task per page), `reference/` (technical/lookup material, not narrative). The user picked this 3-category option over a 4-category variant that also split out a separate "Concepts" bucket.
- **Search: self-hosted orama, not a cloud service** — same "no paid account needed yet" reasoning as GHCR (below) and GitHub Packages.
- **Seeded with the same org-root docs every other repo gets** (`PRD.md`, `ARCHITECTURE.md`, `DESIGN.md`, `AI.md`→`CLAUDE.md`, `MEMORY.md`, `ROADMAP.md`), per the pattern established when `ossplay` itself was seeded.
